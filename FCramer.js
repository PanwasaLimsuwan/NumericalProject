    ...
    let calcarmerCal = () => { //เก็บinput
        ...
        for (k = 0; k < Number(N); k++) { //0 1 2
            for (i = k + 1; i < Number(N); i++) { //1 2
                let temp = array[i][k] / array[k][k] // temp = 10/00 = 2 , 21/11 3/22
                for (j = k + 1; j <= Number(N); j++) { //11 22 33
                    array[i][j] = array[i][j] - temp * array[k][j] // 11 = temp*01 = 2* = 1
                }
            }
        }
        ...
        for (i = 0; i < Number(N); i++) {
            ans.push(<div>x{i + 1}={x[i].toFixed(6)}</div>) //ส่งคำตอบไปหน้าเว็บ
        }
